...
v2 runs the same commands. The pipeline test ([`tests/default.nf.test`](tests/default.nf.test), run with `nf-test` on the test profile) checks that the decompressed v2 outputs have the md5 sums of the v1.0.0 outputs on the same platform: the MAF (both the whole file and its alignment records), the PSL and both 2bit files, which are the same on every platform ([`tests/data/yeast/v1.0.0_md5.txt`](tests/data/yeast/v1.0.0_md5.txt)), and the all.chain, all.pre.chain, noClass.net and net.axt files of v1.0.0 on that platform (`tests/data/yeast/platform/<platform>/v1.0.0_md5.txt`, see [macOS and Linux](#macos-and-linux)). For the MAF header to name the LAST database as v1.0.0 did (`# S_cerevisiae`, not `# lastdb/S_cerevisiae`), the nf-core `last/lastal` module is patched to call lastal with the bare index name ([`last-lastal.diff`](modules/nf-core/last/lastal/last-lastal.diff)). When v2 was written, the medium and far presets, and the pair with target and query swapped, were also checked against CNEr 1.46.0 run by hand, with the same result.

### macOS and Linux

The outputs of v2 are byte-identical to those of v1.0.0 on the same platform. This was checked on the yeast test pair on macOS arm64 and on Linux x86_64, and on a pair of carp chromosomes on Linux x86_64. The two platforms do not give the same bytes from `axtChain` onward, for v1.0.0 as for v2 (both run LAST 1652 and the kent 482 utilities): the 2bit, sizes, MAF and PSL files are identical, but the chains, nets, axt files and liftOver chains differ slightly. On the yeast pair the two platforms make the same chains, but chains of equal score come in a different order, which changes the chain ids and a few choices of the nets. On a pair of carp chromosomes (about 24 Mb each), some chains also differ in content, by 0.0024% of the aligned bases. **Linux x86_64 is the reference platform for production runs and for CI.** Results made on macOS are just as valid, but they are not byte-identical to results made on Linux. The tests compare the platform-dependent files with references made on the same platform ([`tests/data/README.md`](tests/data/README.md)).
